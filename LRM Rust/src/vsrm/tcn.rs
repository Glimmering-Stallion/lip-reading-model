...
pub struct TemporalConvNetConfig {
    ...
    #[config(default = 3)]
    pub kernel_size: usize,       // temporal kernel size for Conv1D layers within a TCN Block [T]

    #[config(default = 4)]
    pub layers: usize,            // number of TCN Block layers within a main TCN
...
impl<B: Backend> TcnBlock<B> {
    ...
    pub fn new(
        ...
        // perform projection if needed to align input to output channels for residual operation
        let projection_needed = in_channels != out_channels;
    ...
    pub fn forward(&self, input: Tensor<B, 3>) -> Tensor<B, 3> {
        ...
        x.add(residual) // pre activation residual sum
    }
}
...
impl<B: Backend> TemporalConvNet<B> {
    ...
    pub fn new(
        ...
        for i in 0..layers {
            ...
            // using bitwise left shifting (2^i)
            let dilation = 1 << i;
